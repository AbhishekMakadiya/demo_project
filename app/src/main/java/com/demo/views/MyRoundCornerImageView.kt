...
class MyRoundCornerImageView : ImageView {
    ...
    constructor(context: Context, attrs: AttributeSet, defStyle: Int = 0) : super(context, attrs, defStyle) {
        ...
        val a = context.obtainStyledAttributes(attrs,
                R.styleable.MyRoundCornerImageView, defStyle, 0)
        var color = Color.TRANSPARENT
        val index = a.getInt(R.styleable.MyRoundCornerImageView_android_scaleType, -1)
        if (index >= 0) {
            scaleType = sScaleTypeArray[index]
        }

        cornerRadius = a.getDimensionPixelSize(
                R.styleable.MyRoundCornerImageView_MyRCICornerRadiusTopLeft, 0).toFloat()
        mRightTopCornerRadius = a.getDimensionPixelSize(
                R.styleable.MyRoundCornerImageView_MyRCICornerRadiusTopRight, 0).toFloat()
        mLeftBottomCornerRadius = a.getDimensionPixelSize(
                R.styleable.MyRoundCornerImageView_MyRCICornerRadiusBottomLeft, 0).toFloat()
        mRightBottomCornerRadius = a.getDimensionPixelSize(
                R.styleable.MyRoundCornerImageView_MyRCICornerRadiusBottomRight, 0).toFloat()

        color = a.getColor(R.styleable.MyRoundCornerImageView_MyRCIBackColor, Color.TRANSPARENT)
        ...
        borderWidth = a.getDimensionPixelSize(
                R.styleable.MyRoundCornerImageView_MyRCIBorderWidth, 0).toFloat()
        if (borderWidth < 0) {
            throw IllegalArgumentException("border width cannot be negative.")
        }

        borderColors = a
                .getColorStateList(R.styleable.MyRoundCornerImageView_MyRCIBorderColor)
        if (borderColors == null) {
            borderColors = ColorStateList.valueOf(DEFAULT_BORDER_COLOR)
        }

        isOval = a.getBoolean(R.styleable.MyRoundCornerImageView_MyRCIOval, false)
        a.recycle()

        val mBackground = DrawableHelper().getRoundedRectangleDrawableWithStroke(context, color, color, cornerRadius.toInt(), mRightTopCornerRadius.toInt(), mLeftBottomCornerRadius.toInt(), mRightBottomCornerRadius.toInt(), 0)
    ...
    internal class SelectableRoundedCornerDrawable(private val mBitmap: Bitmap?, r: Resources) : Drawable() {
        ...
        companion object {
            ...
            private val TAG = "SelectableRoundedCorner"
    ...
    companion object {
        ...
        val TAG = "MyRoundCornerImageView"
